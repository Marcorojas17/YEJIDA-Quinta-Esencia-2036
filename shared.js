/* ═══════════════════════════════════════════════════════════
   YEJIDÁ-KABALAH · shared.js
   Motor común: nav, fondo, helpers, TOC
   ═══════════════════════════════════════════════════════════ */

'use strict';

// ─────── CATÁLOGO DE LAS 20 PORTALES ───────
const PORTALES = [
...
const MENU = [
  ...
  { href:'portales/02-niveles.html', label:'📖 Portales' },
  ...
  { href:'juegos/sefirot.html',   label:'🎮 Juegos' },
  { href:'extras/angel-regente.html', label:'✨ Extras' }
];
...
function buildNav(){
  ...
  const brand = el('a', { href: getRootPath() + 'index.html', class:'brand' },
    ...
    el('span', { class:'brand-name' }, 'YEJIDÁ', el('small', {}, 'Kabaláh Platinum'))
  );
  ...
  MENU.forEach(m => {
    ...
    if(window.location.href.includes(m.href)) a.classList.add('active');
    links.appendChild(a);
  });
...
function getRootPath(){
  ...
  // Detectar subcarpetas conocidas
...
function buildPager(pageId){
  ...
  PORTALES.forEach(p => {
    const d = el('a', {
      ...
      'aria-selected': p.id === pageId ? 'true' : 'false'
    });
    dots.appendChild(d);
  });
...
function initSecurity(){
  ...
  if(window.location.hostname === 'localhost'){
    console.info('[YEJIDÁ] Cargado ·', new Date().toISOString());
  }
}
